...
export default {
  ...
  onModel(model) {
    ...
    if (typeof model.actions !== 'function' && typeof model.actions !== 'object') {
    ...
    for (const actionName of Object.keys(actions)) {
      this.validate([
        [!!actionName.match(/\//), `Invalid action name (${model.name}/${actionName})`],
        [
          typeof actions[actionName] !== 'function' && typeof actions[actionName] !== 'object',
      ...
      if (typeof actions[actionName] === 'function') {
        ...
        this.actions[`${model.name}/${actionName}`] = actions[actionName].bind(this.dispatch[model.name])
        ...
        this.actions[`${model.name}/${actionName}`] = () => this.dispatch(actions[actionName])

        // Bind exposed dispatch[modelName][actionName] to dispatch(actions[actionName])
        this.dispatch[model.name][actionName] = () => this.dispatch(actions[actionName])
      }
    }
  },
  middleware(store) {
    return next => action => {
      if (action.type in this.actions) {
        next(action)
        return this.actions[action.type](action.payload, store.getState(), action.meta)
      } else {
        return next(action)
      }
    }
